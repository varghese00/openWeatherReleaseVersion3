...
const accounts = {
  ...
  logout(request, response) {
   //response.status(200).clearCookie('station', '');
  ...
  authenticate(request, response) {
    const user = userstore.getUserByEmail(request.body.email,request.body.password);
    if (user) {
  ...
  getCurrentUser(request) {
    ...
    return userstore.getUserByEmail(userEmail,userPassword,firstName,lastName,userid);
  },
};
